...
from utils import  get_db_conn, render_graph_image
...
def is_a_question(State: State) -> State:
    template = """
    Determine if the user query is a question.
    The only three possible responses you should give are: "yes", "no", "Security Violation".
    Do not make up any other types.
...
chatbotapp.add_node("is_a_question", is_a_question)
...
chatbotapp.add_edge(START, "is_a_question")
chatbotapp.add_edge("is_a_question", "check_query_classification")
